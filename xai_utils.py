...
def grad_cam_plus(input_model, image, layer_name):
    cls = np.argmax(input_model.predict(image))
    def normalize(x):
        """Utility function to normalize a tensor by its L2 norm"""
        return (x + 1e-10) / (K.sqrt(K.mean(K.square(x))) + 1e-10)
    ...
    # print('loss have shape:')
    # print(loss.shape)
    # print('all_fmap_masks have shape:')
    # print(all_fmap_masks.shape)
    # print('grads_val have shape:')
    # print(grads_val.shape)
    grads_val_2 = grads_val**2
    grads_val_3 = grads_val_2 * grads_val
    eps = 0.000001
    # print('grads_val_2 have shape:')
    # print(grads_val_2.shape)
    # print('grads_val_3 have shape:')
    # print(grads_val_3.shape)
    sum_activations = np.sum(all_fmap_masks, axis)
    # print('sum_activations have shape:')
    # print(sum_activations.shape)
    aij = grads_val_2 / (2 * grads_val_2 + sum_activations[None, None, :] * grads_val_3 + eps)
    # print('aij have shape:')
    # print(aij.shape)
    # print('grads_val are:')
    # print(grads_val)
    # weights = np.mean(grads_val, axis=axis)
    aij = np.where(grads_val != 0, aij, 0)
    weights = np.maximum(grads_val, 0) * aij
    weights = np.sum(weights, axis=axis)
    # print('weight have shape:')
    # print(weights.shape)
    cam = np.dot(all_fmap_masks[0], weights)
    #print (cam)
    H,W= image.shape[1:3]
